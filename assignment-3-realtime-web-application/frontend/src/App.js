...
function App () {
   ...
   */
  useEffect(() => {
    socket.current = io({ path: '/socket.io', transports: ['websocket'] }) // in production - path: '/issues-app/socket.io'
      ...
      <div className="container">
        ...
        <ul className="list-group" id="issue-list">
          {issues.map((issue, index) => {
            return <li className="list-group-item" key={index}><Card onClickUpdateState={onClickUpdateState} issue={issue}/></li>
          })}
        </ul>
      </div>
    </div>
